...
const replaceKey = (oldK: string, newK: string) => {
	model.value = Object.fromEntries(
		Object.entries(model.value).map(([k, v]) => [k === oldK ? newK : k, v]),
	);
};

const updateDragPosition = (event: DragEvent) => {
	if (!dragInfo.value || (event.clientX === 0 && event.clientY === 0)) return;
...
const onDragStart = (event: DragEvent, i: number, k: string) => {
	dragInfo.value = { index: i, key: k, x: 0, y: 0, element: null!, height: 0 };
	updateDragPosition(event);
	event.dataTransfer?.setData("application/json", JSON.stringify(model.value[k]));
...
<template>
	...
	<div class="StructWidget">
			...
			<tbody>
				<tr
					...
					:ref="
						(el) =>
							k === dragInfo?.key &&
							((dragInfo.element = el as HTMLElement),
							(dragInfo.height = (el as HTMLElement).getBoundingClientRect().height))
					"
					class="item"
				...
				>
					...
					<td>
						<Widget :model-value="k" @update:model-value="replaceKey(k, String($event))" />:&nbsp;
					</td>
					<td>
		...
		<form class="new-key-form" @submit.prevent>
			<button
				type="button"
				:disabled="newKey === undefined || newKey in model"
				:title="
